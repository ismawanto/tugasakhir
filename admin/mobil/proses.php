...
if ($_GET['aksi'] == 'tambah') {
    ...
    if ($_FILES['gambar']["error"] > 0) {
        ...
        if (move_uploaded_file($tmp_name, $target_path)) {
            ...
            $data[] = $_POST['merk'];
            $data[] = $_POST['tipe'];
            $data[] = $_POST['transmisi'];
            $data[] = $_POST['harga'];
            $data[] = $_POST['deskripsi'];
            $data[] = $_POST['status'];
            $data[] = $newfilename;

            $sql = "INSERT INTO `mobil`(`no_plat`, `merk`, `tipe`, `transmisi`, `harga`, `deskripsi`, `status`, `gambar`) 
                VALUES (?,?,?,?,?,?,?,?)";
...
if ($_GET['aksi'] == 'edit') {
    ...
    $data[] = $id;
    $sql = "UPDATE mobil SET no_plat= ?, merk=?, tipe=?, transmisi=?, harga=?, deskripsi=?, status=?, gambar=?
        WHERE id_mobil = ?";
    $row = $koneksi->prepare($sql);
    $row->execute($data);
